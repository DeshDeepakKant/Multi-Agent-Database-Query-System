...
def generate_sqlite_query(state: GraphState) -> GraphState:
    ...
    prompt = ChatPromptTemplate.from_template(
        ...
        **Few-shot Examples:**
        ...
        - User Question: What is the status of the project assigned to Desh Deepak Kant?
        - SQL Query: SELECT T1.status FROM projects AS T1 INNER JOIN employees AS T2 ON T1.employee_id = T2.id WHERE LOWER(T2.name) = LOWER('Desh Deepak Kant');
...
def generate_mongodb_query(state: GraphState) -> GraphState:
    ...
    prompt = ChatPromptTemplate.from_template(
        ...
        **Few-shot Examples:**
        ...
        - User Question: Find papers about RAG by an author named Desh Deepak Kant.
        - MongoDB Query: {{"authors": {{"$regex": "Desh Deepak Kant", "$options": "i"}}, "keywords": {{"$regex": "rag", "$options": "i"}}}}
...
def generate_meilisearch_query(state: GraphState) -> GraphState:
    ...
    prompt = ChatPromptTemplate.from_template(
        ...
        Few-shot Examples:
        - User Question: Find support tickets related to MySQL issues raised by Desh Deepak Kant.
        - MeiliSearch Query String: MySQL Desh Deepak Kant

        - User Question: Retrieve all open tickets related to Neo4j.
        - MeiliSearch Query String: open Neo4j

        - User Question: What tickets has Desh Deepak Kant raised about Neo4j and are open?
        - MeiliSearch Query String: Desh Deepak Kant Neo4j open
...
def generate_neo4j_query(state: GraphState) -> GraphState:
    ...
    prompt = ChatPromptTemplate.from_template(
        ...
        - User Question: List all collaborators of Desh Deepak Kant.
        - Cypher Query: MATCH (r:Researcher {{name: 'Desh Deepak Kant'}})-[:COLLABORATES_WITH]->(collaborator:Researcher) RETURN collaborator.name;
        ...
        - User Question: What projects or topics is Desh Deepak Kant working on?
        - Cypher Query: MATCH (r:Researcher {{name: 'Desh Deepak Kant'}})-[:WORKS_ON]->(pt:ProjectOrTopic) RETURN pt.name, pt.domain;
