...
def suggest_refined_query(state: GraphState) -> GraphState:
    ...
    prompt_template_str = """
        ...
        2.  **Partial Matches:** If the user provides a partial term, use operators like CONTAINS, LIKE, or $regex. For names like "Desh Deepak Kant", consider patterns like "Desh.*Kant" or just "Kant" if the first attempt failed.
        ...
        Original User Question: "papers about RAG by Desh Deepak Kant"
        Failed Query: '{{"authors": {{"$regex": "desh deepak kant", "$options": "i"}}, "keywords": {{"$regex": "rag", "$options": "i"}}}}'
        Refined Query (should be your output if this was the scenario):
        REFINED_QUERY_START
        {{"$and": [{{"authors": {{"$regex": "Kant", "$options": "i"}}}}, {{"$or": [{{"title": {{"$regex": "Retrieval-Augmented Generation", "$options": "i"}}}}, {{"title": {{"$regex": "RAG", "$options": "i"}}}}, {{"keywords": {{"$regex": "RAG", "$options": "i"}}}}]}}]}}
        REFINED_QUERY_END
